...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Date
\date{Modified: March 20, 2014.  Compiled: \today}
...
the ordering of two measurements.
...
\begin{enumerate}
  ...
  \item Compute the scores for selected feature pairs
    to identify the top performing TSP;
  \item Identify the number of $K$ TSP to be used in the final classifier;
...
Prognostic information can be extracted from the column names
of the expression matrix, as follows:
...
using the \Rfunction{SWAP.KTSP.Train}.
...
The \Rpackage{swithcBox3} allows to training a KTSP classifier
using a pre-specified set of restricted feature pairs.
This can be useful to implement KTSP classifiers mechanistically
restricted. To achieve this the specific TSP to be used must
be defined using the \Rfunarg{RestrictedPairs}.
As an example we can define a set of specific pairs 
to be used for classifier development by randolmly selecting 
some of the rownames from the \Robject{inputMat} matrix.
In a real example these pairs would  be provided by the user.
The restricted pairs must be encoded using valid names.
...
and aggregates the votes of all $K$ TSP of the classifier.
...
classify one or more samples. 
Below is shown the \textbf{resubstitution} performance in the 
training set.
...
We can apply the trained classifier to a new set of samples,
using an alternative decision rule based 
as specified by \Rfunarg{DecideFunc}.
For instance we can apply the classifier to the test 
counting all votes for class one and then classify 
a patient in one class or the other based on a specific threshold.
